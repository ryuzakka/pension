...
<div id="section">
	...
	<h2>예약 현황</h2>
	<div>사용이 완료된 예약은 보이지 않습니다.<br>전체 내용을 보시려면 '전체보기'를 눌러주세요.</div>
	<a href="reserve_all.jsp">전체보기</a><p>
	...
	<table width="1000" align="center" border="1">
		...
		<tr>
			...
			<td>
				...
				<c:if test="${reserve.state == 1}">취소 신청중</c:if>
				<c:if test="${reserve.state == 2}">취소 완료</c:if>
				<c:if test="${reserve.state == 3}">사용 완료</c:if>
			</td>
			<td>
				<c:if test="${reserve.state == 1}">
					<a href="reserve_cancel.jsp?id=${reserve.id}"><input type="button" value="취소승인"></a>
				</c:if>
			</td>
